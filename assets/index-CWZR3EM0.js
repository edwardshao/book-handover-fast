(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))r(t);new MutationObserver(t=>{for(const e of t)if(e.type==="childList")for(const o of e.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&r(o)}).observe(document,{childList:!0,subtree:!0});function a(t){const e={};return t.integrity&&(e.integrity=t.integrity),t.referrerPolicy&&(e.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?e.credentials="include":t.crossOrigin==="anonymous"?e.credentials="omit":e.credentials="same-origin",e}function r(t){if(t.ep)return;t.ep=!0;const e=a(t);fetch(t.href,e)}})();const q="BookHandoverDB",T=1,m="books",h=()=>new Promise((s,n)=>{const a=indexedDB.open(q,T);a.onerror=r=>{console.error("Database error:",r.target.error),n(r.target.error)},a.onsuccess=r=>{s(r.target.result)},a.onupgradeneeded=r=>{const t=r.target.result;t.objectStoreNames.contains(m)||t.createObjectStore(m,{keyPath:"id",autoIncrement:!0})}}),L=async s=>{const n=await h();return new Promise((a,r)=>{const t=n.transaction([m],"readwrite"),e=t.objectStore(m);t.oncomplete=()=>a(),t.onerror=o=>r(o.target.error),s.forEach(o=>{const i={title:o.title||"Unknown Title",quantity:parseInt(o.quantity)||0,isbns:Array.isArray(o.isbns)?o.isbns:o.isbn?[o.isbn]:[],handedOver:parseInt(o.handedOver)||0,createdAt:new Date().toISOString()};e.add(i)})})},N=async()=>{const s=await h();return new Promise((n,a)=>{const e=s.transaction([m],"readonly").objectStore(m).getAll();e.onsuccess=()=>n(e.result),e.onerror=o=>a(o.target.error)})},B=async(s,n)=>{const a=await h();return new Promise((r,t)=>{const o=a.transaction([m],"readwrite").objectStore(m),i=o.get(s);i.onsuccess=()=>{const c=i.result;c.handedOver=n;const d=o.put(c);d.onsuccess=()=>r(),d.onerror=p=>t(p.target.error)},i.onerror=c=>t(c.target.error)})},P=async()=>{const s=await h();return new Promise((n,a)=>{const e=s.transaction([m],"readwrite").objectStore(m).clear();e.onsuccess=()=>n(),e.onerror=o=>a(o.target.error)})},I=s=>new Promise(n=>setTimeout(n,s)),$=async s=>{const n=localStorage.getItem("cors_proxy_url");if(!n)return console.warn("CORS Proxy URL is not set. Skipping fetch."),"";const a=`${n}?${encodeURIComponent(s)}`,r=5;for(let t=0;t<r;t++)try{const e=await fetch(a);if(e.ok)return await e.text();if((e.status===429||e.status===400)&&(console.warn(`Proxy returned ${e.status}. Retrying (${t+1}/${r})...`),t<r-1)){await I(1e3);continue}throw new Error(`Proxy Error: ${e.status}`)}catch(e){if(t===r-1)throw e;console.warn(`Fetch error: ${e.message}. Retrying (${t+1}/${r})...`),await I(1e3)}},k=async s=>{try{console.log(`Searching Books.com.tw for: ${s}`);const n=`https://search.books.com.tw/search/query/key/${encodeURIComponent(s)}/cat/all`,a=await $(n),r=new DOMParser,e=r.parseFromString(a,"text/html").querySelector('h4 a[href*="item/"]');if(!e)return console.log("No results found on Books.com.tw"),[];const o=e.getAttribute("href"),i=o.match(/item\/([\w\d]+)/);if(!i||!i[1])return console.log("Could not extract item ID from link:",o),[];const d=`https://www.books.com.tw/products/${i[1]}`;console.log(`Constructed detail URL: ${d}`);const p=await $(d),y=r.parseFromString(p,"text/html").body.textContent.match(/ISBN：(\d{13})/);return y&&y[1]?(console.log(`Found ISBN on Books.com.tw: ${y[1]}`),[y[1]]):[]}catch(n){return console.error("Error searching Books.com.tw:",n),[]}},C=async(s,n)=>{try{n&&n("Google Books");const a=localStorage.getItem("google_books_api_key");let r=`https://www.googleapis.com/books/v1/volumes?q=intitle:${encodeURIComponent(s)}&maxResults=5`;a&&(r+=`&key=${a}`);const t=await fetch(r);if(t.status===429)console.warn("Google Books API Rate Limit Exceeded. Trying fallback...");else if(t.ok){const o=await t.json();if(o.items&&o.items.length>0){const i=new Set;if(o.items.forEach(c=>{const d=c.volumeInfo?.industryIdentifiers;d&&d.forEach(p=>{p.type==="ISBN_13"&&i.add(p.identifier)})}),i.size>0)return Array.from(i)}}return localStorage.getItem("cors_proxy_url")?(console.log("Google Books returned no results. Trying Books.com.tw fallback..."),n&&n("Books.com.tw"),await k(s)):(console.log("Google Books returned no results and no CORS Proxy set. Skipping Books.com.tw fallback."),[])}catch(a){return console.error(`Error searching ISBN for "${s}":`,a),localStorage.getItem("cors_proxy_url")?(n&&n("Books.com.tw"),await k(s)):[]}},D=s=>{const n=s.split(/\r?\n/).filter(a=>a.trim()!=="");return n.length===0?[]:(n[0].split(",").map(a=>a.trim().replace(/^"(.*)"$/,"$1")),n.slice(1).map(a=>{const r=[];let t="",e=!1;for(let o=0;o<a.length;o++){const i=a[o];i==='"'?e=!e:i===","&&!e?(r.push(t.trim().replace(/^"(.*)"$/,"$1")),t=""):t+=i}return r.push(t.trim().replace(/^"(.*)"$/,"$1")),{title:r[0]||"",quantity:parseInt(r[1])||0}}).filter(a=>a.title!==""))},O=(s,n)=>{const a=new Blob([JSON.stringify(s,null,2)],{type:"application/json"}),r=URL.createObjectURL(a),t=document.createElement("a");t.href=r,t.download=n,document.body.appendChild(t),t.click(),document.body.removeChild(t),URL.revokeObjectURL(r)};let l=[];document.addEventListener("DOMContentLoaded",async()=>{await h(),await g(),H(),v("convert"),b()});const H=()=>{const s=document.querySelectorAll(".nav-btn");s.forEach(n=>{n.addEventListener("click",()=>{s.forEach(r=>r.classList.remove("active")),n.classList.add("active");const a=n.getAttribute("data-view");v(a)})})},g=async()=>{l=await N(),b()},b=()=>{const s=l.reduce((r,t)=>r+t.quantity,0),n=l.reduce((r,t)=>r+t.handedOver,0),a=s>0?Math.round(n/s*100):0;document.getElementById("overall-progress").textContent=a},v=s=>{const n=document.getElementById("view-container");switch(document.querySelectorAll(".nav-btn").forEach(r=>{r.getAttribute("data-view")===s?r.classList.add("active"):r.classList.remove("active")}),s){case"convert":n.innerHTML=R(),_();break;case"upload":n.innerHTML=A(),U();break;case"with-isbn":n.innerHTML=M();break;case"without-isbn":n.innerHTML=j();break;case"handover":n.innerHTML=z(),K();break}},R=()=>{const n=localStorage.getItem("google_books_api_key")?`
  ...
  `},_=()=>{const s=document.getElementById("csv-upload-box"),n=document.getElementById("csv-file-input"),a=document.getElementById("save-api-key-btn"),r=document.getElementById("clear-api-key-btn"),t=document.getElementById("api-key-input");a&&a.addEventListener("click",()=>{const c=t.value.trim();c&&(localStorage.setItem("google_books_api_key",c),v("convert"))}),r&&r.addEventListener("click",()=>{confirm("確定要清除 API Key 嗎？")&&(localStorage.removeItem("google_books_api_key"),v("convert"))});const e=document.getElementById("save-proxy-url-btn"),o=document.getElementById("clear-proxy-url-btn"),i=document.getElementById("proxy-url-input");e&&e.addEventListener("click",()=>{let c=i.value.trim();if(c){if(!c.startsWith("http://")&&!c.startsWith("https://")){alert("請輸入包含 http:// 或 https:// 的完整 URL");return}localStorage.setItem("cors_proxy_url",c),v("convert")}}),o&&o.addEventListener("click",()=>{confirm("確定要清除 CORS Proxy URL 嗎？")&&(localStorage.removeItem("cors_proxy_url"),v("convert"))}),s.addEventListener("click",()=>n.click()),n.addEventListener("change",async c=>{const d=c.target.files[0];d&&await S(d)}),s.addEventListener("dragover",c=>{c.preventDefault(),s.style.borderColor="var(--accent-color)"}),s.addEventListener("dragleave",()=>{s.style.borderColor="var(--border-color)"}),s.addEventListener("drop",async c=>{c.preventDefault(),s.style.borderColor="var(--border-color)";const d=c.dataTransfer.files[0];d&&d.name.endsWith(".csv")&&await S(d)})},S=async s=>{const n=document.getElementById("convert-result");n.innerHTML='<div class="spinner"></div><p>正在讀取 CSV 並查詢 ISBN...</p>';const a=await s.text(),r=D(a);if(r.length===0){n.innerHTML='<p style="color: var(--danger-color);">❌ CSV 檔案格式錯誤或無資料</p>';return}const t=[];for(let o=0;o<r.length;o++){const i=r[o],c=await C(i.title,d=>{n.innerHTML=`<div class="spinner"></div>
                                   ...
                                   <p style="font-size: 0.85rem; color: var(--text-secondary);">書名: ${i.title}</p>`});t.push({title:i.title,quantity:i.quantity,isbns:c}),await new Promise(d=>setTimeout(d,300))}let e="<h3>✅ 轉換完成</h3>";e+='<table class="data-table"><thead><tr><th>書名</th><th>數量</th><th>ISBN</th></tr></thead><tbody>',t.forEach(o=>{const i=o.isbns.length>0?o.isbns.join(", "):"查無 ISBN";e+=`<tr><td>${o.title}</td><td>${o.quantity}</td><td>${i}</td></tr>`}),e+="</tbody></table>",e+='<div style="margin-top: 1.5rem;">',e+='<button class="btn" id="download-json-btn" style="margin-right: 1rem;">💾 下載 JSON 檔案</button>',e+='<button class="btn btn-primary" id="import-db-btn">📥 直接匯入資料庫</button>',e+="</div>",n.innerHTML=e,document.getElementById("download-json-btn").addEventListener("click",()=>{const o=new Date().toISOString().split("T")[0];O(t,`book-handover-list-${o}.json`)}),document.getElementById("import-db-btn").addEventListener("click",async()=>{const o=t.map(i=>({title:i.title,quantity:parseInt(i.quantity)||0,isbns:Array.isArray(i.isbns)?i.isbns:[],handedOver:0}));await L(o),await g(),v("upload")})},A=()=>`
    <div class="view-card">
      <h2>📥 上傳點交系統書單</h2>
  ...
  `,U=()=>{const s=document.getElementById("json-upload-box"),n=document.getElementById("json-file-input"),a=document.getElementById("clear-db-btn"),r=document.getElementById("download-db-btn");r&&r.addEventListener("click",()=>{const t=l.map(o=>({title:o.title,quantity:o.quantity,isbns:o.isbns})),e=new Date().toISOString().split("T")[0];O(t,`book-handover-list-${e}.json`)}),a&&a.addEventListener("click",async()=>{confirm("確定要清空所有書籍資料嗎？此操作無法復原！")&&(await P(),await g(),v("upload"))}),s.addEventListener("click",()=>n.click()),n.addEventListener("change",async t=>{const e=t.target.files[0];e&&await E(e)}),s.addEventListener("dragover",t=>{t.preventDefault(),s.style.borderColor="var(--accent-color)"}),s.addEventListener("dragleave",()=>{s.style.borderColor="var(--border-color)"}),s.addEventListener("drop",async t=>{t.preventDefault(),s.style.borderColor="var(--border-color)";const e=t.dataTransfer.files[0];e&&e.name.endsWith(".json")&&await E(e)})},E=async s=>{const n=document.getElementById("upload-result");try{const a=await s.text(),r=JSON.parse(a);if(!Array.isArray(r))throw new Error("JSON 格式錯誤：必須是陣列");const t=r.map(e=>({title:e.title,quantity:parseInt(e.quantity)||0,isbns:Array.isArray(e.isbns)?e.isbns:[],handedOver:0}));await L(t),await g(),n.innerHTML=`<p style="color: var(--success-color);">✅ 成功上傳 ${t.length} 筆書籍資料！</p>`,setTimeout(()=>v("upload"),1500)}catch(a){n.innerHTML=`<p style="color: var(--danger-color);">❌ 上傳失敗: ${a.message}</p>`}},M=()=>{const s=l.filter(e=>e.isbns&&e.isbns.length>0),n=s.filter(e=>e.handedOver<e.quantity),a=s.filter(e=>e.handedOver>=e.quantity),r=n.reduce((e,o)=>e+(o.quantity-o.handedOver),0),t=a.reduce((e,o)=>e+o.handedOver,0);return`
    <div class="view-card">
      <h2>✅ 有 ISBN 的書籍</h2>
    ...
    <div class="handover-view">
      <div class="split-left">
        <div class="book-list-section">
          ...
          <div id="pending-books">${f(s,!0,"pending")}</div>
        </div>
        ...
        <div class="book-list-section">
          ...
          <div id="completed-books">${f(n,!0,"completed")}</div>
        </div>
      </div>
  ...
  `},K=()=>{const s=document.getElementById("isbn-input"),n=document.getElementById("match-result"),a=document.getElementById("pending-books"),r=document.getElementById("completed-books"),t=()=>{const o=l.filter(u=>u.handedOver<u.quantity),i=l.filter(u=>u.handedOver>=u.quantity);document.getElementById("pending-books").innerHTML=f(o,!0,"pending"),document.getElementById("completed-books").innerHTML=f(i,!0,"completed");const c=o.reduce((u,y)=>u+(y.quantity-y.handedOver),0),d=i.reduce((u,y)=>u+y.handedOver,0),p=document.getElementById("pending-stats"),x=document.getElementById("completed-stats");p&&(p.textContent=`(${o.length} 款 / 共 ${c} 本)`),x&&(x.textContent=`(${i.length} 款 / 共 ${d} 本)`)},e=async o=>{if(o.target.classList.contains("btn-handover")){const i=parseInt(o.target.getAttribute("data-book-id")),c=l.find(d=>d.id===i);if(c){let d;o.target.classList.contains("btn-increment")?d=c.handedOver+1:o.target.classList.contains("btn-decrement")&&(d=Math.max(0,c.handedOver-1)),d!==void 0&&(await B(c.id,d),await g(),t(),b())}}};a&&a.addEventListener("click",e),r&&r.addEventListener("click",e),s.addEventListener("keypress",async o=>{if(o.key==="Enter"){const i=s.value.trim();if(!i)return;const c=l.find(d=>d.isbns&&d.isbns.some(p=>p===i));if(c&&c.handedOver<c.quantity){const d=c.handedOver+1;await B(c.id,d),await g(),n.innerHTML=`
        ...
        `,t(),b()}else c&&c.handedOver>=c.quantity?n.innerHTML=`
